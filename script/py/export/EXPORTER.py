...
def setContainer(src=parent.Comp, name=f"{parent.Comp.name}_export"):
    ...
    if container == None:
        ...
        container.color = tuple([x * 0.75 for x in list(src.color)])
    ...
    for p in inputsPage:
        if p.name in pFilterList:
            inputsFilteredPage.append(
                parameterToJSONPar(p, extraAttrs="*", forceAttrLists=True))
    ...
    for node in nodes:
        if node.name.startswith(nodeNamesToCopy):
            ...
            if n.name in ["BUILTINS_INIT"]:
                n.nodeY = 2000
            if (n.name.startswith(("renderselect", "cache"))
                    and n.digits not in activeOutputs):
    ...
    if container.op("FACTORY_MACROS") != None:
        ...
        macroFactoryOP.par.syncfile = False
        macroFactoryOP.par.file = ""
