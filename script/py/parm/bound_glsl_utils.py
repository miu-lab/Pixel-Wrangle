...
def getParGroups(parmList, namesRow, selectedColName='pargroup'):
    if type(selectedColName) == int:
        rmDouble = {
            k[selectedColName].val: v for v, k in enumerate(reversed(parmList))
        }
...
def setAtomicCounters(target, fromCodeParms, bindMasterRelativePath, GLacParOffset, GLacParGroups):
...
def setVulkanConstants(target, bindMasterRelativePath, GLconstParOffset, GLconstParGroups):
    for i, parGroup in enumerate(GLconstParGroups):
        ...
        setExpression(
            pGroupName, tarpName, tarpVal, bindMasterRelativePath
        )
...
def setArrays(target, fromCodeParms, bindMasterRelativePath, GLarrayParOffset, GLarrayParGroups):
...
def setParameterSection(fromCodeParms, parGroups, GLuniParGroups, GLarrayParGroups, GLmatParGroups, GLacParGroups, GLconstParGroups):
