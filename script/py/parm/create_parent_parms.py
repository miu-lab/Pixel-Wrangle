...
def addParFromJSON(
    ...
    return addParameterFromJSONDict(
        target, src, replace, setValues, ignoreAttrErrors, fixParNames, setBuiltIns
    )
...
def onTableChange(dat: DAT):
    ...
    # SORT PAGES
    target.sortCustomPages("Controls", "Code", "Inputs", "Outputs", "GLSL", "Globals")
    ...
    for index, _ in enumerate(plist):
        ...
        pSettings = {
            ...
            "startSection": int(dat[i, "section"].val)
            if dat[i, "section"].val != "None"
            else 0,
            "readOnly": int(dat[i, "readonly"].val)
            if dat[i, "readonly"].val != "None"
            else 0,
            ...
            "enableExpr": dat[i, "enableExpr"].val
            if dat[i, "enableExpr"].val != "None"
            else "",
            "expr": f"{dat[i, 'expr'].val}" if dat[i, "expr"].val != "None" else "",
            "bindExpr": dat[i, "bindExpr"].val
            if dat[i, "bindExpr"].val != "None"
            else "",
            "min": min,
            "max": max,
