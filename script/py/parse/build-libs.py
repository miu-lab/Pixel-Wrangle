...
def getUniquePathList(dat, relPathColumn="relpath"):
    ...
    pathList.sort()
    return ["/".join(x) for x in [l for l, _ in itertools.groupby(pathList)]]
...
def createFolderStructure(pathList, parent="/libs"):
    ...
    for path in pathList:
        ...
        for i in range(maxDepth):
            ...
            if op(f"{parent}/{currentPath}") is None:
...
def createFiles(root, dat):
    ...
    for i, row in enumerate(rows):
        ...
        if targetOP is None:
            ...
            formatedFilePath = (
                f"{parent.Comp.par.Codelibrarypath}/Functions/dist/"
                + str(dat[i + 1, "relpath"].val)
            )
